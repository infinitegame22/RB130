...
Goal: intake an array and take a sequence of elements 2 at a time, yielding each pair of elements to the block and returning `nil`.
input: array
output: nil

Rules:
- use #each, #each_with_object, #each_with_index, #inject, loop, for, while, or until
=end

def each_cons(array)
  array.each_with_index.with_object([]) do |(ele, index), result|
    result << yield(ele, array[index+1]) if array[index+1]
  end
  nil
end
